...
export default function Alunos() {
  ...
  async function load() {
    ...
    setError(null)
    try {
      const { data, error: e } = await getSupabase().from('alunos').select('*').order('dia_pagamento')
      if (e) throw e
      setAlunos(data || [])
    } catch (e: any) {
  ...
  async function salvar() {
    ...
    setSaving(true)
    try {
      await getSupabase().from('alunos').insert({ nome: form.nome, modalidade: form.modalidade, valor: parseFloat(form.valor), frequencia: form.frequencia, dia_pagamento: parseInt(form.dia_pagamento) || 1, ativo: true, obs: form.obs })
      await getSupabase().from('receitas').insert({ nome: form.nome, tipo: form.modalidade, sub_tipo: 'fixa', valor: parseFloat(form.valor), dia: parseInt(form.dia_pagamento) || 1, status: 'previsto', recorrente: true, obs: 'Criado via cadastro de aluno' })
    } finally {
      setSaving(false)
      setModal(false)
      setForm(empty)
  ...
  return (
    <main style={{ padding: 16 }}>
      ...
      <Grid3>
        <KpiCard label="Ativos"        value={String(ativos.length)}  color="blue" />
      ...
      <Card>
        {ativos.sort((a, b) => a.dia_pagamento - b.dia_pagamento).map(a => (
          <Row key={a.id}>
            <div style={{ width: 36, height: 36, borderRadius: 10, background: a.modalidade === 'consultoria' ? 'var(--blue-bg)' : 'var(--green-bg)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, fontWeight: 700, color: a.modalidade === 'consultoria' ? 'var(--blue-text)' : 'var(--green-text)', flexShrink: 0 }}>{initials(a.nome)}</div>
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 13, fontWeight: 500 }}>{a.nome}</div>
              <div style={{ fontSize: 11, color: 'var(--text2)' }}>{a.modalidade} · {a.frequencia} · dia {a.dia_pagamento}</div>
            </div>
            <Badge variant={a.modalidade === 'consultoria' ? 'blue' : 'green'}>{a.modalidade}</Badge>
            <span style={{ color: 'var(--green-text)', fontWeight: 600, fontSize: 13, minWidth: 72, textAlign: 'right' }}>{fmt(a.valor)}</span>
            <Btn size="sm" variant="danger" onClick={() => toggleAtivo(a.id, a.ativo)}>Inativar</Btn>
          </Row>
        ))}
        {ativos.length === 0 && <div style={{ color: 'var(--text3)', textAlign: 'center', padding: 20 }}>Nenhum aluno ativo</div>}
      </Card>
      {inativos.length > 0 && (
        <>
          ...
          <Card>
            {inativos.map(a => (
              <Row key={a.id}>
                <div style={{ width: 36, height: 36, borderRadius: 10, background: 'var(--bg4)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, fontWeight: 700, color: 'var(--text3)', flexShrink: 0 }}>{initials(a.nome)}</div>
                <div style={{ flex: 1 }}><div style={{ fontSize: 13, fontWeight: 500, color: 'var(--text2)' }}>{a.nome}</div><div style={{ fontSize: 11, color: 'var(--text3)' }}>{a.modalidade}</div></div>
      ...
      {modal && (
        <Modal title="Novo Aluno" onClose={() => setModal(false)}>
          <FormRow>
            ...
            <FormGroup label="Modalidade"><select value={form.modalidade} onChange={f('modalidade')}><option value="personal">Personal</option><option value="consultoria">Consultoria</option></select></FormGroup>
          </FormRow>
          <FormRow>
            <FormGroup label="Valor (R$)"><input type="number" value={form.valor} onChange={f('valor')} placeholder="0.00" inputMode="decimal" /></FormGroup>
            <FormGroup label="Frequência"><input value={form.frequencia} onChange={f('frequencia')} placeholder="3x/semana" /></FormGroup>
