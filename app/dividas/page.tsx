...
export default function Dividas() {
  ...
  async function load() {
    ...
    try {
      const { data, error: e } = await getSupabase().from('dividas').select('*').order('prioridade')
      if (e) throw e
      setDividas(data || [])
  ...
  async function pagar(id: string, valor: number) {
    ...
    const novoPago = Math.min(d.valor_total, d.pago + valor)
    await getSupabase().from('dividas').update({ pago: novoPago, atrasada: novoPago >= d.valor_total ? false : d.atrasada }).eq('id', id)
    load()
  }
  async function deletar(id: string) { if (!confirm('Remover?')) return; await getSupabase().from('dividas').delete().eq('id', id); load() }
  async function salvar() {
    if (!form.nome || !form.valor_total) return alert('Preencha nome e valor.')
    setSaving(true)
    try { await getSupabase().from('dividas').insert({ nome: form.nome, credor: form.credor, valor_total: parseFloat(form.valor_total), valor_parcela: parseFloat(form.valor_parcela) || 0, vencimento: parseInt(form.vencimento) || 1, atrasada: form.atrasada === 'true', prioridade: parseInt(form.prioridade) || 3, obs: form.obs, pago: 0 }) }
  ...
  return (
    <main style={{ padding: 16 }}>
      ...
      <Grid3>
        <KpiCard label="Total" value={fmtShort(restante)} color="red" />
        <KpiCard label="Quitado" value={fmtShort(pago)} color="green" />
        <KpiCard label="Atrasadas" value={`${atrasadas}`} color="amber" />
      </Grid3>
      {pago > 0 && <Card style={{ marginTop: 12 }}><div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}><span style={{ fontSize: 12, color: 'var(--text2)' }}>Progresso de Quitação</span><span style={{ fontSize: 12, fontWeight: 600, color: 'var(--green-text)' }}>{pct}%</span></div><ProgressBar pct={pct} /></Card>}
      <SectionTitle>Lista por Prioridade</SectionTitle>
      <Card>
          ...
          return (
            <div key={d.id} style={{ padding: '12px 0', borderBottom: '1px solid var(--border)', display: 'flex', flexDirection: 'column', gap: 8 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                <div style={{ flex: 1 }}><div style={{ fontSize: 13, fontWeight: 500 }}>{d.nome}</div><div style={{ fontSize: 11, color: 'var(--text2)' }}>{d.credor} · Venc. dia {d.vencimento}</div></div>
                <Badge variant={d.atrasada && restD > 0 ? 'red' : restD === 0 ? 'green' : 'gray'}>{d.atrasada && restD > 0 ? 'atrasada' : restD === 0 ? 'quitada' : 'em dia'}</Badge>
              ...
              <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                ...
                {restD > 0 ? (<><Btn size="sm" onClick={() => pagar(d.id, d.valor_parcela)}>Pagar parcela</Btn><Btn size="sm" variant="primary" onClick={() => pagar(d.id, restD)}>Quitar</Btn><Btn size="sm" variant="danger" onClick={() => deletar(d.id)}>✕</Btn></>) : (<><Badge variant="green">Quitada!</Badge><Btn size="sm" variant="danger" onClick={() => deletar(d.id)}>✕</Btn></>)}
              </div>
            </div>
          )
        })}
      ...
      {modal && (
        <Modal title="Nova Dívida" onClose={() => setModal(false)}>
          ...
          <FormRow>
            <FormGroup label="Valor total (R$)"><input type="number" value={form.valor_total} onChange={f('valor_total')} placeholder="0.00" inputMode="decimal" /></FormGroup>
            <FormGroup label="Parcela (R$)"><input type="number" value={form.valor_parcela} onChange={f('valor_parcela')} placeholder="0.00" inputMode="decimal" /></FormGroup>
            <FormGroup label="Venc. (dia)"><input type="number" value={form.vencimento} onChange={f('vencimento')} placeholder="Dia" min="1" max="31" /></FormGroup>
          </FormRow>
          <FormRow>
            <FormGroup label="Atrasada?"><select value={form.atrasada} onChange={f('atrasada')}><option value="false">Não</option><option value="true">Sim</option></select></FormGroup>
