...
export default function Dashboard() {
  ...
  useEffect(() => {
    async function load() {
      const [{ data: r }, { data: d }, { data: dv }] = await Promise.all([
        getSupabase().from('receitas').select('*').order('dia'),
        getSupabase().from('despesas').select('*').order('dia'),
  ...
  useEffect(() => {
    ...
    chartInst.current = new Chart(chartRef.current, {
      type: 'bar',
      data: { labels: proj.map(p => String(p.day)), datasets: [{ label: 'Saldo', data: vals, backgroundColor: vals.map(v => v < 0 ? 'rgba(239,68,68,.8)' : 'rgba(59,130,246,.8)'), borderRadius: 4 }] },
  ...
  }, [loading, receitas, despesas, dividas])
  ...
  return (
    <main style={{ padding: 16 }}>
      ...
      {dividasAtraso.length > 0 && <Alert variant="amber"><span>!</span><span>{dividasAtraso.length} dívida(s) em atraso — Total: {fmt(dividasAtraso.reduce((a, d) => a + (d.valor_total - d.pago), 0))}</span></Alert>}
      <Grid4>
        ...
        <KpiCard label="Livre p/ Gastar" value={fmtShort(fin.livre)}        sub="Caixa menos comprometido" color={fin.livre >= 0 ? 'green' : 'red'} />
      ...
      <Card style={{ marginTop: 12 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 }}>
          ...
          <Badge variant={minSaldo >= 0 ? 'green' : 'red'}>{minSaldo >= 0 ? 'Sem risco' : 'Risco ' + fmt(minSaldo)}</Badge>
        </div>
