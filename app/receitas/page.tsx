...
export default function Receitas() {
  ...
  async function load() {
    ...
    try {
      const { data, error: e } = await getSupabase().from('receitas').select('*').order('dia')
      if (e) throw e
      setReceitas(data || [])
  ...
  async function toggleStatus(id: string, s: string) { await getSupabase().from('receitas').update({ status: s }).eq('id', id); load() }
  async function deletar(id: string) { if (!confirm('Remover?')) return; await getSupabase().from('receitas').delete().eq('id', id); load() }
  async function salvar() {
    if (!form.nome || !form.valor || !form.dia) return alert('Preencha nome, valor e dia.')
    setSaving(true)
    try { await getSupabase().from('receitas').insert({ nome: form.nome, tipo: form.tipo, sub_tipo: form.sub_tipo, valor: parseFloat(form.valor), dia: parseInt(form.dia), status: form.status, recorrente: form.recorrente === 'true', obs: form.obs }) }
  ...
  return (
    <main style={{ padding: 16 }}>
      ...
      <Grid2>
        <KpiCard label="Renda Fixa" value={fmtShort(totalF)} sub={`Recebido: ${fmt(recebF)} · Previsto: ${fmt(totalF - recebF)}`} color="green" />
        <KpiCard label="Renda Variável" value={fmtShort(totalV)} sub={`Recebido: ${fmt(recebV)} · Previsto: ${fmt(totalV - recebV)}`} color="blue" />
      </Grid2>
      ...
      <Card>
        {fixas.sort((a, b) => a.dia - b.dia).map(r => (
          <Row key={r.id}>
            ...
            <div style={{ flex: 1 }}><div style={{ fontSize: 13, fontWeight: 500 }}>{r.nome}</div><div style={{ fontSize: 11, color: 'var(--text2)' }}>{r.tipo}{r.recorrente ? ' · mensal' : ''}</div></div>
            {badge(r)}
            <span style={{ color: 'var(--green-text)', fontWeight: 600, fontSize: 13, minWidth: 72, textAlign: 'right' }}>{fmt(r.valor)}</span>
            <div style={{ display: 'flex', gap: 4 }}>
              {r.status !== 'recebido' ? <Btn size="sm" onClick={() => toggleStatus(r.id, 'recebido')}>✓</Btn> : <Btn size="sm" onClick={() => toggleStatus(r.id, 'previsto')}>↩</Btn>}
              <Btn size="sm" variant="danger" onClick={() => deletar(r.id)}>✕</Btn>
            </div>
          </Row>
        ))}
      </Card>
      <SectionTitle>Renda Variável</SectionTitle>
      <Card>
        {variaveis.length === 0 ? <div style={{ color: 'var(--text3)', textAlign: 'center', padding: 20 }}>Nenhuma renda variável lançada</div>
          : variaveis.sort((a, b) => a.dia - b.dia).map(r => (
            <Row key={r.id}>
              ...
              <div style={{ flex: 1 }}><div style={{ fontSize: 13, fontWeight: 500 }}>{r.nome}</div><div style={{ fontSize: 11, color: 'var(--text2)' }}>{r.tipo}{r.obs ? ' · ' + r.obs : ''}</div></div>
              {badge(r)}
              <span style={{ color: 'var(--blue-text)', fontWeight: 600, fontSize: 13, minWidth: 72, textAlign: 'right' }}>{fmt(r.valor)}</span>
              <div style={{ display: 'flex', gap: 4 }}>
                {r.status !== 'recebido' ? <Btn size="sm" onClick={() => toggleStatus(r.id, 'recebido')}>✓</Btn> : <Btn size="sm" onClick={() => toggleStatus(r.id, 'previsto')}>↩</Btn>}
                <Btn size="sm" variant="danger" onClick={() => deletar(r.id)}>✕</Btn>
              </div>
      ...
      {modal && (
        <Modal title="Nova Receita" onClose={() => setModal(false)}>
          <FormRow>
            ...
            <FormGroup label="Tipo"><select value={form.tipo} onChange={f('tipo')}><option value="personal">Personal</option><option value="consultoria">Consultoria</option><option value="renovação">Renovação</option></select></FormGroup>
          </FormRow>
          <FormRow>
          ...
          <FormRow>
            <FormGroup label="Status"><select value={form.status} onChange={f('status')}><option value="previsto">Previsto</option><option value="recebido">Recebido</option><option value="atrasado">Atrasado</option></select></FormGroup>
            <FormGroup label="Recorrente"><select value={form.recorrente} onChange={f('recorrente')}><option value="true">Sim</option><option value="false">Não</option></select></FormGroup>
          </FormRow>
